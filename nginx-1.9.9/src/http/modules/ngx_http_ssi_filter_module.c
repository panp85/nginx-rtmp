...
ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ngx_log_error(1, r->connection->log, 0,
					   "panpan test, in ngx_http_ssi_body_filter, http postpone filter \"%V?%V\" %p", &r->uri, &r->args, in);
...
ngx_http_ssi_stub_output(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
	...
	ngx_log_error(1, r->connection->log, 0, 
		"panpan test, in ngx_http_ssi_stub_output, go to ngx_http_output_filter.\n");
