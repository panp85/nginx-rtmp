...
ngx_http_header_filter(ngx_http_request_t *r)
{
        ...
        && r->headers_out.location->value.data[0] == '/')
    {
        ...
        if (clcf->server_name_in_redirect) {
			ngx_log_error(1, c->log, 0,
		            "panpan test, in ngx_http_header_filter_init, clcf->server_name_in_redirect yes.\n");
			...
			ngx_log_error(1, c->log, 0,
				    "panpan test, in ngx_http_header_filter_init, r->headers_in.server.len yes.\n");

        } else {
            host.len = NGX_SOCKADDR_STRLEN;
            host.data = addr;
			ngx_log_error(1, c->log, 0,
					"panpan test, in ngx_http_header_filter_init, r->headers_in.server.len no.\n");
        ...
        port = 0;
		ngx_log_error(1, c->log, 0,
		            "panpan test, in ngx_http_header_filter_init, line 386.\n");
    }
    ...
    *b->last++ = CR; *b->last++ = LF;
    ...
    ngx_log_error(1, c->log, 0,
                   "panpan test, in ngx_http_header_filter, %*s", (size_t) (b->last - b->pos), b->pos);
...
{
    ngx_log_error(1, cf->log, 0,
                   "panpan test, in ngx_http_header_filter_init, set ngx_http_top_header_filter to ngx_http_header_filter.\n");
