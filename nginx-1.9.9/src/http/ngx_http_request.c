...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    c->log->data = ctx;
    c->log->action = "waiting for request, in ngx_http_init_connection.";
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    c = rev->data;
    ngx_log_error(1, c->log, 0, "panpan test, in ngx_http_wait_request_handler, go in.\n");
    ...
    ngx_log_error(1, c->log, 0,
                      "panpan test, in ngx_http_wait_request_handler, recv: %s.\n", b->last);
    if (n == NGX_AGAIN) {
    ...
    if (hc->proxy_protocol) {
        hc->proxy_protocol = 0;
		ngx_log_error(1, c->log, 0,
							  "panpan test, in ngx_http_wait_request_handler, proxy_protocol ok.\n");
        ...
        if (b->pos == b->last) {
            c->log->action = "waiting for request, in ngx_http_wait_request_handler.";
            b->pos = b->start;
            b->last = b->start;
            ngx_post_event(rev, &ngx_posted_events);
            return;
	...
	{
	    ngx_log_error(1, c->log, 0,
							  "panpan test, in ngx_http_wait_request_handler, proxy_protocol no.\n");
	}
...
ngx_http_ssl_handshake(ngx_event_t *rev)
{
    ...
    ngx_log_error(1, rev->log, 0,
                      "panpan test, in ngx_http_ssl_handshake, recv: %s.\n", buf);
    ...
    if (hc->proxy_protocol) {
        ...
        ngx_log_error(1, rev->log, 0,
                      "panpan test, in ngx_http_ssl_handshake 2, recv: %s.\n", buf);
        c->log->action = "SSL handshaking";
    ...
    if (n == 1) {
        ...
        c->log->action = "waiting for request, in ngx_http_ssl_handshake";
...
ngx_http_ssl_handshake_handler(ngx_connection_t *c)
{
    if (c->ssl->handshaked) {
     ...
     && (defined TLSEXT_TYPE_application_layer_protocol_negotiation           \
        ...
        c->log->action = "waiting for request, in ngx_http_ssl_handshake_handler.";
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    ngx_log_error(1, rev->log, 0,
                   "panpan test, in ngx_http_process_request_headers, go in.\n");
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (rev->ready) {
		...
		ngx_log_error(1, c->log, 0,
                      "panpan test, in ngx_http_read_request_header, recv: %s.\n", r->header_in->last);
    } else {
        ngx_log_error(1, c->log, 0,
                      "panpan test, in ngx_http_read_request_header, rev->ready no.\n");
        n = NGX_AGAIN;
    }
...
ngx_http_writer(ngx_http_request_t *r)
{
	...
	ngx_log_error(1, r->connection->log, 0, 
					"panpan test, in ngx_http_writer, go to ngx_http_output_filter.\n");
...
ngx_http_test_reading(ngx_http_request_t *r)
{
    ...
    ngx_log_error(1, c->log, 0,
                      "panpan test, in ngx_http_test_reading, recv: %s.\n", buf);
    if (n == 0) {
        rev->eof = 1;
        c->error = 1;
        err = 0;
...
ngx_http_keepalive_handler(ngx_event_t *rev)
{
    ...
    n = c->recv(c, b->last, size);
	ngx_log_error(1, c->log, 0,
                      "panpan test, in ngx_http_test_reading 2, recv: %s.\n",  b->last);
    c->log_error = NGX_ERROR_INFO;
...
ngx_http_lingering_close_handler(ngx_event_t *rev)
{
    ...
    do {
        ...
        ngx_log_error(1, c->log, 0,
                      "panpan test, in ngx_http_test_reading 3, recv: %s.\n", buffer);
...
ngx_http_send_special(ngx_http_request_t *r, ngx_uint_t flags)
{
    ...
    out.next = NULL;
	ngx_log_error(1, r->connection->log, 0, 
				"panpan test, in ngx_http_send_special, go to ngx_http_output_filter.\n");
