...
ngx_http_upstream_init(ngx_http_request_t *r)
{
    ...
    ngx_log_error(1, c->log, 0,
                   "panpan test, in ngx_http_upstream_init, http init upstream, client timer: %d", c->read->timer_set);
    ...
    ngx_event_t *ev)
{
	...
	ngx_log_error(1, ev->log, 0,
						 "panpan test, in ngx_http_upstream_check_broken_connection, recv: %s.\n", buf);
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_error(1, c->log, 0,
						 "panpan test, in ngx_http_upstream_process_header, recv: %s.\n", u->buffer.last);
        if (n == NGX_AGAIN) {
#if 0
    ...
    ngx_http_upstream_t *u)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_error(1, c->log, 0,
						 "panpan test, in ngx_http_upstream_process_body_in_memory, recv: %s.\n", b->last);
        if (n == NGX_AGAIN) {
            break;
        }
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    p = u->pipe;
	ngx_log_error(1, c->log, 0,
					   "panpan test, in ngx_http_upstream_send_response, set p->output_filter to ngx_http_output_filter.\n");
    ...
    ngx_uint_t from_upstream, ngx_uint_t do_write)
{
    ...
    for ( ;; ) {
        ...
        if (size && src->read->ready) {
			...
			ngx_log_error(1, c->log, 0,
									"panpan test, in ngx_http_upstream_process_upgraded, recv: %s.\n", b->last);
    ...
    for ( ;; ) {
        ...
        if (do_write) {
            ...
            if (u->out_bufs || u->busy_bufs) {
				ngx_log_error(1, upstream->log, 0,
                                  "panpan test, in ngx_http_upstream_process_non_buffered_request, go to ngx_http_output_filter.\n");
        ...
        if (size && upstream->read->ready) {
            ...
            ngx_log_error(1, upstream->log, 0,
									"panpan test, in ngx_http_upstream_process_non_buffered_request, recv: %s.\n", b->last);
            if (n == NGX_AGAIN) {
                break;
            }
